...
public class HCartViewHolder extends RecyclerView.ViewHolder {
    ...
    public void bindData(CartItem cartItem, IViewHolderListeners listeners) {
        ...
        binding.tvPrice.setText(String.format("%d đ", cartItem.getPrice()));
        binding.tvQty.setText(String.valueOf(cartItem.getQuantity()));

        // 3. Events
        binding.btnMinus.setOnClickListener(v -> listeners.onItemModifyQuantity(cartItem.getProductId(), -1));
        binding.btnPlus.setOnClickListener(v -> listeners.onItemModifyQuantity(cartItem.getProductId(), 1));
    }
}
